...
def onOrUnpack(request):
    ...
    if processname:
        ...
        deviceutil.spawn_process_and_load_script_file(
            processname, "./server/fridaJavaScript/orUnpack.js"
        )
...
def onAdUnpack(request):
    ...
    if processname:
        ...
        deviceutil.setup_device(devices)
        deviceutil.spawn_process_and_load_script_file(
            processname, "./server/fridaJavaScript/adUnpack.js"
        )
        return HttpResponse("OK")
...
def onOrNativeHook(request):
    ...
    orNativeHookJs = formatJS.formatOrNativeHook(
        moduleName, exportName, enlogcode, lelogcode
    )
    deviceutil = DeviceUtil()
    devices = fridaFunc.enmuDevices()
    deviceutil.setup_device(devices)
...
def onAdNativeHook(request):
    ...
    enlogcode = request.POST.get("enlogcode")
    lelogcode = request.POST.get("lelogcode")
    onAdNativeHookjs = formatJS.formatAdNativeHook(
        moduleName, instruction, funcoffset, enlogcode, lelogcode
    )
    # print(nativeHookJs)
    deviceutil = DeviceUtil()
    devices = fridaFunc.enmuDevices()
...
def onIosFuncHook(request):
    ...
    enlogcode = request.POST.get("enlogcode")
    lelogcode = request.POST.get("lelogcode")
    onIosFuncHookJs = formatJS.formatIosFuncHook(
        classname, funcname, enlogcode, lelogcode
    )
    deviceutil = DeviceUtil()
    devices = fridaFunc.enmuDevices()
    deviceutil.setup_device(devices)
...
def enmuModules(request):
    ...
    deviceutil.attach_process_and_load_script_file(
        "./server/fridaJavaScript/enmuModules.js"
    )
    return HttpResponse("OK")
...
def enumerateLoadedClasses(request):
    ...
    deviceutil.setup_process(processname)
    deviceutil.attach_process_and_load_script_file(
        "./server/fridaJavaScript/enumLoadedClasses.js"
    )
    return HttpResponse("OK")
